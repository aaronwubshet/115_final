...
#if !defined(SensorInterrupt__REMOVED) /* Check for removal by optimization */
...
void SensorInterrupt_Start(void)
{
    ...
    /* Set the priority. */
    SensorInterrupt_SetPriority((uint8)SensorInterrupt_INTC_PRIOR_NUMBER);
...
void SensorInterrupt_Stop(void)
{
    ...
    SensorInterrupt_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef SensorInterrupt_INTERRUPT_INTERRUPT_CALLBACK
        SensorInterrupt_Interrupt_InterruptCallback();
    #endif /* SensorInterrupt_INTERRUPT_INTERRUPT_CALLBACK */ 
...
*   before the component has been started use SensorInterrupt_StartEx instead.
* 
*   When defining ISR functions, the CY_ISR and CY_ISR_PROTO macros should be 
*   used to provide consistent definition across compilers:
*
...
void SensorInterrupt_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)SensorInterrupt__INTC_NUMBER] = address;
}
...
cyisraddress SensorInterrupt_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)SensorInterrupt__INTC_NUMBER];
}
...
*
*   Note calling SensorInterrupt_Start or SensorInterrupt_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after SensorInterrupt_Start or SensorInterrupt_StartEx has been called. 
...
uint8 SensorInterrupt_GetState(void)
{
    ...
    return ((*SensorInterrupt_INTC_SET_EN & (uint32)SensorInterrupt__INTC_MASK) != 0u) ? 1u:0u;
}
