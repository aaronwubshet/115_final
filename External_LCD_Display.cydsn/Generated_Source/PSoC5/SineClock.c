...
void SineClock_Start(void) 
{
    ...
    SineClock_CLKEN |= SineClock_CLKEN_MASK;
	SineClock_CLKSTBY |= SineClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SineClock_Stop
********************************************************************************
*
* Summary:
...
void SineClock_Stop(void) 
{
    ...
    SineClock_CLKEN &= (uint8)(~SineClock_CLKEN_MASK);
	SineClock_CLKSTBY &= (uint8)(~SineClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SineClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SineClock_CLKEN & SineClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SineClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SineClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SineClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SineClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SineClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SineClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SineClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SineClock_GetSourceRegister();
    uint16 oldDivider = SineClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SineClock_CLKEN & SineClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SineClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SineClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SineClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SineClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SineClock_MOD_SRC |= modeBitMask & (uint8)SineClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SineClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SineClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SineClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SineClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SineClock_MOD_SRC & (uint8)(SineClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SineClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SineClock_GetDividerRegister();
    uint8 oldSrc = SineClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SineClock_MOD_SRC |= CYCLK_SSS;
        SineClock_MOD_SRC =
            (SineClock_MOD_SRC & (uint8)(~SineClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SineClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SineClock_MOD_SRC & SineClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SineClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SineClock_PHASE = clkPhase & SineClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SineClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SineClock_PHASE & SineClock_PHASE_MASK;
}
