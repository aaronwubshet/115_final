/*******************************************************************************
* File Name: SensorScan_Clock_int.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_Start
********************************************************************************
*
* Summary:
...
void SensorScan_Clock_int_Start(void) 
{
    ...
    SensorScan_Clock_int_CLKEN |= SensorScan_Clock_int_CLKEN_MASK;
	SensorScan_Clock_int_CLKSTBY |= SensorScan_Clock_int_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_Stop
********************************************************************************
*
* Summary:
...
void SensorScan_Clock_int_Stop(void) 
{
    ...
    SensorScan_Clock_int_CLKEN &= (uint8)(~SensorScan_Clock_int_CLKEN_MASK);
	SensorScan_Clock_int_CLKSTBY &= (uint8)(~SensorScan_Clock_int_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SensorScan_Clock_int_CLKEN & SensorScan_Clock_int_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SensorScan_Clock_int_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SensorScan_Clock_int_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SensorScan_Clock_int__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SensorScan_Clock_int_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SensorScan_Clock_int_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SensorScan_Clock_int_GetSourceRegister();
    uint16 oldDivider = SensorScan_Clock_int_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SensorScan_Clock_int_CLKEN & SensorScan_Clock_int_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SensorScan_Clock_int_DIV_PTR, clkDivider);
                SensorScan_Clock_int_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SensorScan_Clock_int_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SensorScan_Clock_int_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SensorScan_Clock_int_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SensorScan_Clock_int_DIV_PTR, clkDivider);
				SensorScan_Clock_int_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SensorScan_Clock_int_DIV_PTR);
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SensorScan_Clock_int_MOD_SRC |= modeBitMask & (uint8)SensorScan_Clock_int_MODE_MASK;
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SensorScan_Clock_int_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SensorScan_Clock_int_MODE_MASK));
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SensorScan_Clock_int_MOD_SRC & (uint8)(SensorScan_Clock_int_MODE_MASK);
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SensorScan_Clock_int_GetDividerRegister();
    uint8 oldSrc = SensorScan_Clock_int_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SensorScan_Clock_int_MOD_SRC |= CYCLK_SSS;
        SensorScan_Clock_int_MOD_SRC =
            (SensorScan_Clock_int_MOD_SRC & (uint8)(~SensorScan_Clock_int_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SensorScan_Clock_int_MOD_SRC & SensorScan_Clock_int_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SensorScan_Clock_int_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SensorScan_Clock_int_PHASE = clkPhase & SensorScan_Clock_int_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SensorScan_Clock_int_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SensorScan_Clock_int_PHASE & SensorScan_Clock_int_PHASE_MASK;
}
